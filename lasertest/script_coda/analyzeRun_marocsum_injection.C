void analyzeRun_marocsum_injection(int vme_run,int ichip=1) {
...
//injector run for chip1 and chip2
        int nint_a[n] = { 12, 6, 6, 6, 6, 6 };
        int nint_b[n] = { 26, 20, 20,20,20,20 };
        int nped_a[n] = { 0, 0, 0, 0, 0, 0 };
        int nped_b[n] = { 6, 6, 6, 6, 6, 6 };
	...
	for( int ipixel = 0; ipixel < 8; ipixel++ ) {
                hinjection[i][ipixel] = new TH1F( Form("hinjection%i_%i",i,ipixel), ";injection amp; mean of integral fadc counts (pedestal subtracted)", 4100, 0., 4100. );
	}
	}

                TH1I *hintSub_tmp[n];
                for( int i = 0; i < n; i++ ) hintSub_tmp[i] = new TH1I(Form("hintSub_tmp%i",i), "Integral (pedestal subtracted)",  30500, -500., 30000. );
	...
	T->SetBranchAddress( "wave"  , &wave  );
                        ...
                        int off;
                        if (ichip==2) off=3;
                       	else if (ichip==1) off=8;
                       	else {cout << "which chip?" << endl; return;}

			cout << "chip " << ichip << " with offset " << off << endl;
...
//	for( int ievt = 2; ievt < T->GetEntries()-11; ievt++ ) {
      for( int ipixel = 0; ipixel < 8; ipixel++ ) {
	//cout << ievt+1 << endl;
	for( int icharge = 0; icharge < 9; icharge++ ) {
		 //cout << ievt+1 << endl; 
	 for( int icount = 0; icount < 1000; icount++ ) {
	  	
		ievt=2+(ipixel)*9*1000+(icharge)*1000+icount+ipixel*10;
		//continue; 
...
//	  	if( ievt%1000 == 0 ) cout << "Processing event " << ievt << endl;
		...
		for( int i = 1; i < n; i++ ) {
	   		...
	   		for( int isamp = nped_a[i]; isamp < nped_b[i]; isamp++ ) {
				if( i < 2 ) loc_ped += wave[i+off][isamp];
				else loc_ped += 4095 - wave[i+off][isamp];
			}
				...
				double f_wave;
	      			if( i < 2 ) 
					f_wave = static_cast<double>( wave[i+off][isamp] );
	      			else
					f_wave = static_cast<double>( 4095 - wave[i+off][isamp] );
		...
		for( int i = 1; i < n; i++ ) {
			hinjection[i][ipixel]->Fill(icharge*500+50+50*i,hintSub_tmp[i]->GetMean(1));
			hintSub_tmp[i]->Reset();
		}
...
//	string label[n] = { "sum64", "dynode", "quad1", "quad2", "quad3", "quad4" };
	...
	hintSub_2D->GetYaxis()->SetRangeUser( 0., 10000. );
        ...
        TCanvas *c8 = new TCanvas( "c8", "injection", 1400, 1000 );
        c8->SetTickx(); c8->SetTicky(); c8->cd();	
        for( int i = 1; i < n; i++ ) {
	  for( int ipixel = 0; ipixel < 8; ipixel++ ) {
		hinjection[i][ipixel]->SetMarkerStyle(20);
		hinjection[i][ipixel]->SetMarkerSize(2);
		hinjection[i][ipixel]->SetMarkerColor(i+1);
		hinjection[i][ipixel]->SetLineColor(i+1);
		
		if (i==1 && ipixel==0) {hinjection[i][ipixel]->Draw("P hist"); hinjection[i][ipixel]->SetMaximum(4000);}
		else hinjection[i][ipixel]->Draw("same P hist");
	}
	}
        TLegend *leg1 = new TLegend( 0.2, 0.95 , 0.4, 0.7 );
        for( int i=1; i<n; i++ ) leg1->AddEntry( hinjection[i][0], label[i].c_str(), "l" );
        leg1->Draw();
...
//	c7->Print( Form("data/fevme1_%06d.pdf)", vme_run), "pdf" );
	...
	c8->Print( Form("data/fevme1_%06d.pdf)", vme_run), "pdf" );
