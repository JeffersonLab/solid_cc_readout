...
//	int nint_a[n] = { 20, 20, 20, 20, 20, 20 };
	...
	*/
	...
	TH2I *hintSub_2D= new TH2I( "hintSub_2D", "Integral (pedestal subtracted) ; quadsum; sum64", 20500, -500., 20000., 20500, -500., 20000.);
	...
	for( int i=0; i<n; i++ ) leg->AddEntry( hped[i], label[i].c_str(), "l" );
	...
	hamp[0]->SetMaximum( 1.1*max );  hamp[0]->Draw();
	...
	hint[0]->SetMaximum( 1.1*max );  hint[0]->Draw();
	...
	hampSub[0]->SetMaximum( 1.1*max ); hampSub[0]->Draw();
	...
	hintSub[0]->SetMaximum( 1.1*max ); hintSub[0]->Draw();
	...
	for( int i = 0; i < n; i++ ) {
		...
		hint[i]->GetXaxis()->SetRangeUser(     10000,  50000  );
//		hampSub[i]->GetXaxis()->SetRangeUser( -200.,   5000. );
//		hintSub[i]->GetXaxis()->SetRangeUser( -200.,   15000. );
              hampSub[i]->GetXaxis()->SetRangeUser( -400.,  1000. );
              hintSub[i]->GetXaxis()->SetRangeUser( -400.,  2000. );

	}
	hampSub_2D->GetXaxis()->SetRangeUser( 1000.,  5000. );
	hampSub_2D->GetYaxis()->SetRangeUser( 1000.,  5000. );	
	
	hintSub_2D->GetXaxis()->SetRangeUser( 1000., 10000. );
	hintSub_2D->GetYaxis()->SetRangeUser( 1000., 10000. );
