...
void ana_fadc(int vme_run,int thisevent=10)
{
...
//  int tw=96;
  TH1I *pH[9];
  pH[0] = new TH1I("h1","fadc0_marocsum;ns;adc*Nevent",tw, 0,4*tw);
  pH[1] = new TH1I("h2","fadc1;ns;adc*Nevent",tw, 0,4*tw);
  pH[2] = new TH1I("h3","fadc2;ns;adc*Nevent",tw, 0,4*tw);
  pH[3] = new TH1I("h4","fadc3_sum64;ns;adc*Nevent",tw, 0,4*tw);
  pH[4] = new TH1I("h5","fadc4_dynode;ns;adc*Nevent",tw, 0,4*tw);
  pH[5] = new TH1I("h6","fadc5_quad1;ns;adc*Nevent",tw, 0,4*tw);
  pH[6] = new TH1I("h7","fadc6_quad2;ns;adc*Nevent",tw, 0,4*tw);
  pH[7] = new TH1I("h8","fadc7_quad3;ns;adc*Nevent",tw, 0,4*tw);
  pH[8] = new TH1I("h9","fadc8_quad4;ns;adc*Nevent",tw, 0,4*tw);
...
//  for(Long64_t entry=10;entry<11;entry++)
  for(Long64_t entry=thisevent;entry<thisevent+1;entry++)
  {
//    if(!(entry%10000)) printf("%lld ", entry);
...
//	 else pH[i]->Fill(4*j,fadcEvent.wave[i][j]);
	 if (i<3 || i>4) pH[i]->Fill(4*j,4095-fadcEvent.wave[i][j]);
	 else pH[i]->Fill(4*j,fadcEvent.wave[i][j]);
